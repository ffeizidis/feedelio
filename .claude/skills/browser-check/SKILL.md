...
This is **exploratory**. `e2e/tests/` is the CI regression suite and belongs to CI: never
add to it, edit it, or run scenarios from it. Scenarios you write here live in a scratch
directory and are thrown away.
...
The three worked examples below all pass today:
...
A scenario is one Playwright spec file. Copy the closest example from
`.claude/skills/browser-check/scenarios/` into a scratch directory (**not** into this
repo — use your own temp/scratchpad dir), edit it, and run it.
...
- **Scenarios in one run share one library** and run one at a time, in file-name order.
  Either make a scenario tolerant of what the others left behind (`expect([201,
  409]).toContain(...)`), or run it on its own.
...
| `scenarios/*.spec.ts` | The three worked examples, also usable as templates. |
